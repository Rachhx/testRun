...
// used to how skeleton when detected
...
namespace AstraTestWpf
{
    ...
    internal sealed class BodyVisualizer
    {
        ...
        /// 
        double firstPos, secPos, diff;

        //noted,rachel: add counter
        public int counter = 0;
        public int timeval = 0;
        public int timeframe = 10;
        ...
        private void DrawBodies()
        {
            lock (bodies)
            {
                using (var dc = drawingGroup.Open())
                {
                    ...
                    // to show the counter that the system is running 
                    timeval++;
                    if (timeval % timeframe == 0)
                    {
                        counter++;
                    }
                    var txt = new FormattedText(counter.ToString(), CultureInfo.InvariantCulture, FlowDirection.LeftToRight, Fonts.SystemTypefaces.First(), JointLabelFontSizeEm, JointLabelShadow);
                    ...
                    txt.SetFontSize(25);
                    dc.DrawText(txt, new Point(depthWidth / 2, depthHeight /8));
